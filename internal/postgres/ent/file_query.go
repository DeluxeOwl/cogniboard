...
func (fq *FileQuery) QueryTask() *TaskQuery {
	query := (&TaskClient{config: fq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, file.TaskTable, file.TaskPrimaryKey...),
		)
...
func (fq *FileQuery) WithTask(opts ...func(*TaskQuery)) *FileQuery {
	query := (&TaskClient{config: fq.config}).Query()
...
func (fq *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := fq.withTask; query != nil {
		if err := fq.loadTask(ctx, query, nodes,
			func(n *File) { n.Edges.Task = []*Task{} },
			func(n *File, e *Task) { n.Edges.Task = append(n.Edges.Task, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FileQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*File, init func(*File), assign func(*File, *Task)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(file.TaskTable)
		s.Join(joinT).On(s.C(task.FieldID), joinT.C(file.TaskPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(file.TaskPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(file.TaskPrimaryKey[1]))
