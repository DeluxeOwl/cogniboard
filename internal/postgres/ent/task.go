...
func (e TaskEdges) FilesOrErr() ([]*File, error) {
	if e.loadedTypes[0] {
		return e.Files, nil
	}
...
func (*Task) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case task.FieldID, task.FieldTitle, task.FieldDescription, task.FieldAssigneeName, task.FieldStatus:
			values[i] = new(sql.NullString)
		case task.FieldDueDate, task.FieldCreatedAt, task.FieldUpdatedAt, task.FieldCompletedAt:
	...
	for i := range columns {
		...
		case task.FieldAssigneeName:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				t.AssigneeName = new(string)
				*t.AssigneeName = value.String
			}
...
// QueryFiles queries the "files" edge of the Task entity.
func (t *Task) QueryFiles() *FileQuery {
	return NewTaskClient(t.config).QueryFiles(t)
}
